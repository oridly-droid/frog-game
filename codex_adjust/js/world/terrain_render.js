...
function getTerrainContactPalette(terrainType){
    if(terrainType === "mud"){
        return {
            base:"rgba(113,133,112,0.2)",
            fringe:"rgba(112,164,151,0.16)",
            glow:"rgba(174,209,184,0.13)",
            shadow:"rgba(12,22,20,0.1)"
        }
    }
    if(terrainType === "danger"){
        return {
            base:"rgba(105,96,122,0.18)",
            fringe:"rgba(137,105,150,0.14)",
            glow:"rgba(174,138,158,0.1)",
            shadow:"rgba(18,15,22,0.1)"
        }
    }
    return {
        base:"rgba(84,135,104,0.18)",
        fringe:"rgba(109,178,137,0.15)",
        glow:"rgba(183,210,153,0.09)",
        shadow:"rgba(12,22,18,0.085)"
    }
}
...
export function drawFlower(context, flower){
    ...
    context.fillStyle = "rgba(16,30,22,0.14)"
    context.beginPath()
    context.ellipse(0, flower.size * 0.72, flower.size * 1.22, flower.size * 0.34, 0, 0, Math.PI * 2)
    context.fill()
    ...
    for(let i = 0; i < flower.petals; i++){
        ...
        context.fillStyle = "rgba(255,255,238,0.22)"
        context.beginPath()
        context.ellipse(px - Math.cos(angle) * flower.size * 0.12, py - Math.sin(angle) * flower.size * 0.12, flower.size * 0.34, flower.size * 0.18, angle, 0, Math.PI * 2)
        context.fill()
    }

    const pollenGlow = context.createRadialGradient(0, 0, flower.size * 0.08, 0, 0, flower.size * 0.9)
    pollenGlow.addColorStop(0, "rgba(255,248,174,0.38)")
    pollenGlow.addColorStop(1, "rgba(255,248,174,0)")
    context.fillStyle = pollenGlow
    context.beginPath()
    context.arc(0, 0, flower.size * 0.9, 0, Math.PI * 2)
    context.fill()
...
function drawCodexCanopyShadows(context){
    if(terrainRenderProfile.mobileOptimized){
        return
    }

    context.save()
    context.globalAlpha = 0.18
    context.fillStyle = "rgba(7,32,26,0.18)"
    for(let i = 0; i < 34; i++){
        const x = (world.width * ((i * 0.137) % 1) + 180) % world.width
        const y = (world.height * ((i * 0.219) % 1) + 120) % world.height
        const rx = 110 + (i % 7) * 22
        const ry = 38 + (i % 5) * 12
        context.save()
        context.translate(x, y)
        context.rotate((i % 11 - 5) * 0.13)
        context.beginPath()
...
function drawCodexWaterHighlights(context){
    context.save()
    context.lineCap = "round"
    context.lineJoin = "round"

    for(const pool of terrain.shallowPools || []){
        context.strokeStyle = "rgba(204,249,238,0.22)"
        context.lineWidth = 3.2
        traceSmoothPath(context, pool.points)
        context.stroke()

        const bounds = getPointsBounds(pool.points)
        const centerX = (bounds.minX + bounds.maxX) * 0.5
        const centerY = (bounds.minY + bounds.maxY) * 0.5
        const rx = Math.max(18, (bounds.maxX - bounds.minX) * 0.5)
        const ry = Math.max(8, (bounds.maxY - bounds.minY) * 0.5)
        for(let i = 0; i < 3; i++){
            context.strokeStyle = `rgba(226,255,242,${0.16 - i * 0.03})`
            context.lineWidth = 1.4
            context.beginPath()
            context.ellipse(centerX, centerY, rx * (0.24 + i * 0.16), ry * (0.2 + i * 0.12), 0, 0.2, Math.PI * 1.72)
            context.stroke()
        }
    }

    context.strokeStyle = "rgba(181,230,226,0.12)"
    context.lineWidth = 5
    for(let i = 0; i < 9; i++){
        const y = world.height * (0.18 + i * 0.085)
        const startX = world.width * (0.58 + (i % 3) * 0.035)
        context.beginPath()
        context.moveTo(startX, y)
        context.bezierCurveTo(startX + 120, y + 34, startX + 280, y - 42, startX + 460, y + 18)
        context.stroke()
    }
...
function drawCodexFireflies(context){
    context.save()
    for(let i = 0; i < 70; i++){
        const x = (world.width * ((i * 0.61803398875) % 1) + 90) % world.width
        const y = (world.height * ((i * 0.41421356237) % 1) + 130) % world.height
        const r = 1.2 + (i % 5) * 0.35
        const glow = context.createRadialGradient(x, y, 0, x, y, r * 5.6)
        glow.addColorStop(0, "rgba(235,255,179,0.34)")
        glow.addColorStop(0.32, "rgba(190,245,174,0.13)")
        glow.addColorStop(1, "rgba(190,245,174,0)")
        context.fillStyle = glow
        context.beginPath()
        context.arc(x, y, r * 5.6, 0, Math.PI * 2)
        context.fill()
    }
    context.restore()
}

function drawCodexScenicDetails(context){
    drawCodexCanopyShadows(context)
    drawCodexWaterHighlights(context)
    drawCodexFireflies(context)
}
...
export function renderBackgroundLayer(){
    ...
    meadowGradient.addColorStop(0, "#6fc86d")
    meadowGradient.addColorStop(0.38, "#4fa85f")
    meadowGradient.addColorStop(0.74, "#347f56")
    meadowGradient.addColorStop(1, "#225f45")
    ...
    drawCodexScenicDetails(backgroundCtx)
    ...
    vignette.addColorStop(1, "rgba(8,31,30,0.2)")
